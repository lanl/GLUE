...
def mergeBufferTable(solverCode, sqlDB):
    if solverCode == SolverCode.BGK:
        sqlCursor = sqlDB.cursor()
        mergeStr = "MERGE BGKRESULTS AS TARGET USING BGKFASTRESULTS AS SOURCE WHEN MATCHED BY TARGET THEN DELETE WHEN NOT MATCHED BY TARGET THEN INSERT (TAG, RANK, REQ, VISCOSITY, THERMAL_CONDUCT, DIFFCOEFF_0, DIFFCOEFF_1, DIFFCOEFF_2, DIFFCOEFF_3, DIFFCOEFF_4, DIFFCOEFF_5, DIFFCOEFF_6, DIFFCOEFF_7, DIFFCOEFF_8, DIFFCOEFF_9, PROVENANCE) VALUES (SOURCE.TAG, SOURCE.RANK, SOURCE.REQ, SOURCE.VISCOSITY, SOURCE.THERMAL_CONDUCT, SOURCE.DIFFCOEFF_0, SOURCE.DIFFCOEFF_1, SOURCE.DIFFCOEFF_2, SOURCE.DIFFCOEFF_3, SOURCE.DIFFCOEFF_4, SOURCE.DIFFCOEFF_5, SOURCE.DIFFCOEFF_6, SOURCE.DIFFCOEFF_7, SOURCE.DIFFCOEFF_8, SOURCE.DIFFCOEFF_9, SOURCE.PROVENANCE);"
