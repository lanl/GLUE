...
class sm:
    ...
    def eta2(n1, n2, m1, m2, Z1, Z2, T):
        ...
        etatot = (x1 * x1 * R1 + x2 * x2 * R2 + x1 * x2 * R12p) / (
                x1 * x1 * R1 / eta_1 + x2 * x2 * R2 / eta_2 + x1 * x2 * R12)
    ...
    def Ktherm2(n1, n2, m1, m2, Z1, Z2, T):
        ...
        K = (x1 * x1 * Q1 * K1 + x2 * x2 * Q2 * K2 + x1 * x2 * Q12p) / (
                x1 * x1 * Q1 + x2 * x2 * Q2 + x1 * x2 * Q12)  # 1 / cm s
    ...
    def K11(g):
        ...
        if g < 1.0:
            return -0.25 * np.log(
                1.466 * g - 1.7836 * g ** 2.0 + 1.4313 * g ** 3.0 - 0.55833 * g ** 4 + 0.061162 * g ** 5)
        else:
    ...
    def K12(g):
        ...
        if g < 1.0:
            return -0.25 * np.log(
                0.52094 * g + 0.25153 * g ** 2.0 - 1.1337 * g ** 3.0 + 1.2155 * g ** 4 - 0.43784 * g ** 5)
        else:
...
def ICFAnalytical_solution(LammpsDens, LammpsCharges, LammpsTemperature):
    ...
    for i in species_with_non_zeros_densities_index:
        if len(species_with_non_zeros_densities_index) == 2:
            conductivity_coefficient = Ktherm2(LammpsDens[0], LammpsDens[1], m[0], m[1], LammpsCharges[0],
                                               LammpsCharges[1], LammpsTemperature)
            viscosity_coefficient = eta2(LammpsDens[0], LammpsDens[1], m[0], m[1], LammpsCharges[0], LammpsCharges[1],
                                         LammpsTemperature)
            D_11 = D_ij(LammpsDens[0], LammpsDens[0], m[0], m[0], LammpsCharges[0], LammpsCharges[0], LammpsTemperature,
                        kappa=-1)
            D_12 = D_ij(LammpsDens[0], LammpsDens[1], m[0], m[1], LammpsCharges[0], LammpsCharges[1], LammpsTemperature,
                        kappa=-1)
            D_22 = D_ij(LammpsDens[1], LammpsDens[1], m[1], m[1], LammpsCharges[1], LammpsCharges[1], LammpsTemperature,
                        kappa=-1)

        for idx in species_with_zeros_densities_index:
            if idx == 1:
...
def write_LammpsScript(Temperature, densities, charges, masses, system_index_species, box, cutoff, Teq, Trun,
    ...
    with open(lammpsFName, 'w') as LammpsScript:
        ...
        for s in range(len(densities)):
            ...
            csv_writer.writerow(
                ['fix', str(s + 1), 'atom' + str(s + 1), 'vector', '1', 'c_vacf' + str(index_species[s]) + '[4]'])
            csv_writer.writerow(
                ['variable', 'Diff_' + str(s + 1), 'equal', '1.e4*dt*trap(f_' + str(s + 1) + ')' + '/3'])
        ...
        csv_writer.writerow(
            ['variable', 'v11', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[3]' + ')' + '*${scale_factor_v}'])
        csv_writer.writerow(
            ['variable', 'v22', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[4]' + ')' + '*${scale_factor_v}'])
        csv_writer.writerow(
            ['variable', 'v33', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[5]' + ')' + '*${scale_factor_v}'])
        ...
        csv_writer.writerow(['fix', str(s + 1 + len(densities)), 'all', 'ave/correlate', s_int, p_int, d_int, '&'])
        csv_writer.writerow(
            ['c_flux[1]', 'c_flux[2]', 'c_flux[3]', 'type', 'auto', 'file', 'profile.heatflux.dat', 'ave', 'running'])
        csv_writer.writerow(
            ['variable', 'k11', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[3]' + ')' + '*${scale_factor_k}'])
        csv_writer.writerow(
            ['variable', 'k22', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[4]' + ')' + '*${scale_factor_k}'])
        csv_writer.writerow(
            ['variable', 'k33', 'equal', 'trap(f_' + str(s + 1 + len(densities)) + '[5]' + ')' + '*${scale_factor_k}'])
        ...
        for s in nspecies:
            for j in nspecies:
                if (s == j):
                    csv_writer.writerow(
                        ['variable', 'Diff_' + str(index_species[s - 1]) + str(index_species[j - 1]), 'equal',
                         'v_Diff_' + str(j)])
        ...
        for s in nspecies:
            for j in nspecies:
                if (s == j):
                    csv_writer.writerow(
                        ['print', 'D=${Diff_' + str(index_species[s - 1]) + str(index_species[j - 1]) + '}', 'file',
                         'diffusion_coefficient_' + str(index_species[s - 1]) + str(index_species[j - 1]) + '.csv'])
...
def check_zeros_trace_elements(Temperature, densities, charges, masses, box, cutoff, Teq, Trun, s_int, p_int, d_int,
    ...
    for s in traces_elements_index:
        ...
        traceScript = write_LammpsScript(Temperature, dens, Z, m, b, box, cutoff, Teq, Trun, traces_elements_index,
                                         s_int, p_int, d_int, dirPrefix)
    ...
    for s in non_traces_elements_index:
        ...
        if N == 1:
            ...
            b.append('mixture_' + str(non_traces_elements_index[0]) + str(non_traces_elements_index[1]) + str(
                non_traces_elements_index[2]))
        elif N == 4:
            b.append('mixture_' + str(non_traces_elements_index[0]) + str(non_traces_elements_index[1]) + str(
                non_traces_elements_index[2]) + str(non_traces_elements_index[3]))
        else:
            print('We only have trace and/or zeros species')

    # Generate LAMMPS script after removing zeros and trace elements
    mixtureScript = write_LammpsScript(Temperature, densities[non_traces_elements_index],
                                       charges[non_traces_elements_index], masses[non_traces_elements_index], b, box,
                                       cutoff, Teq, Trun, non_traces_elements_index, s_int, p_int, d_int, dirPrefix)
...
def write_output_coeff(densities0, species_with_zeros_densities_index):
    ...
    for i in nspecies:
        ...
        ### Use the self-diffusions above with the species concentrations to build
        ### mutual diffusion with Darken and dump into files
